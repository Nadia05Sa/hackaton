...
import { lugaresTruequeData, catalogosData } from "../../../data/datosEstaticos";
...
const schema = yup.object().shape({
  ...
  horario: yup.string().required("Requerido"),
});
...
export default function EditarLugarTrueque() {
  const { id } = useParams();
  ...
  useEffect(() => {
    setTipos(catalogosData.tiposLugarTrueque.listarActivos());
    setTimeout(() => {
      const data = lugaresTruequeData.obtenerPorId(id);
      if (data) {
        setLugar(data);
        setImagenPreview(data.imagen);
        reset({ nombre: data.nombre, descripcion: data.descripcion, ubicacion: data.ubicacion, direccion: data.direccion, tipo: data.tipoId?.toString(), horario: data.horario });
      }
      setLoading(false);
    }, 300);
  }, [id, reset]);
  ...
  const onSubmit = (data) => {
    ...
    setTimeout(() => {
      ...
      lugaresTruequeData.actualizar(id, { ...data, tipo: tipoObj?.nombre, tipoId: parseInt(data.tipo), imagen: imagenPreview });
      setSaving(false);
      Swal.fire({ title: "¡Actualizado!", icon: "success", timer: 1500, showConfirmButton: false }).then(() => navigate("/lugares-trueque"));
    }, 500);
  };
    ...
    <Fade in={true} timeout={600}>
      <Box sx={{ pb: 4, maxWidth: 800, mx: "auto" }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
          ...
          <Box><Typography variant="h5" sx={{ fontWeight: 700, color: colors.primary, display: "flex", alignItems: "center", gap: 1 }}><StorefrontIcon sx={{ color: colors.primary }} />Editar Punto de Trueque</Typography></Box>
        </Box>
          ...
          <CardContent sx={{ p: 3 }}>
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end", mt: 3 }}>
                ...
                <Button type="submit" variant="contained" startIcon={saving ? <CircularProgress size={20} color="inherit" /> : <SaveIcon />} disabled={saving} sx={{ background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.primaryLight} 100%)`, borderRadius: 2, height: 48, px: 4, fontWeight: 600 }}>{saving ? "Guardando..." : "Guardar"}</Button>
              </Box>
            </form>
          </CardContent>
        </Card>
