...
const SeleccionarEmpleado = () => {
    ...
    const seleccionar = (id) => {
        navigate('datosAsignacion', { state: { idEmpleado: id, idsLista: idsLista } });
    };
    ...
    if (location.pathname.includes('datosAsignacion')) {
        return <Outlet />;
    }
    return (
        <>
            <Box className="mt-3">
                <Box sx={{ display: 'flex', justifyContent: isXs ? 'center' : 'space-between', mb: isXs ? 2 : 0, alignItems: 'center', flexWrap: 'wrap', gap: 2, }}>
                    ...
                    <Box className="botones-superiores" sx={{ paddingRight: isXs ? 0 : "3rem" }}>
                        <Button variant="contained" onClick={() => cancelar()}>Regresar</Button>
                    </Box>
                </Box>
                        ...
                        <TableContainer component={Paper} sx={{ mt: 3, width: isXs ? "100%" : null, minWidth: isXs ? 280 : 300 }}>
                            ...
                            {contenido ? (
                                    ...
                                    <Box sx={{
                                        ...
                                        {visibleRows.map((item, index) => (
                                            <Card sx={{ width: isXs ? "100%" : 320 }} key={item.id}>
                                                <CardActionArea onClick={() => seleccionar(item.id)} sx={{ bgcolor: "#41514D", borderRadius: 5 }}>
                                                    <CardContent>
                                ...
                                >
                                    ...
                                    <TableBody>
                                        {visibleRows.map((item, index) => (
                                            <TableRow hover tabIndex={-1} key={item.id} onClick={() => isXs ? seleccionar(item.id) : null}>
                                                ...
                                                {!isXs &&
                                                    <TableCell align='center'>
                                                        <Tooltip title="Seleccionar empleado">
                                                            <IconButton onClick={() => seleccionar(item.id)} color="primary">
                                                                <NavigateNextIcon />
                                                            </IconButton>
                                                        </Tooltip>
                                                    </TableCell>
